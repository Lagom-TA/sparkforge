...
- **版本与分享**：提供历史版本浏览与恢复、完整源码文件导航、可交互的公开演示与可撤销的分享链接；公开演示不读取项目业务数据。HTML 源码可下载独立运行，CRUD 应用下载包含构建配置的完整 ZIP 工程；下载副本使用本机存储。
...
审查团队反馈的逐项处理与验证边界见 [2026-09-10 修复记录](docs/reviews/2026-09-10-review-feedback-fix.md)。恢复历史版本会保留业务记录；若旧结构无法使用现有记录，系统会拒绝恢复。HTML 应用使用各版本各自的原有存档。

完整修复、云端 v40 与真实模型验收证据见 [2026-09-11 完整验收记录](docs/reviews/2026-09-11-full-repair-acceptance.md)。
