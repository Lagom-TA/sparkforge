from services.project_integrity import require_project
...
# ------------------ Service Layer ------------------
class Share_linksService:
    """Service layer for Share_links operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None, *, commit: bool = True) -> Optional[Share_links]:
        """Create a new share_links"""
        try:
            await require_project(self.db, data.get("project_id"), user_id)
            if user_id:
                data['user_id'] = user_id
            obj = Share_links(**data)
            self.db.add(obj)
            if commit:
            ...
            logger.info(f"Created share_links with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating share_links: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for share_links {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Share_links]:
        """Get share_links by ID (user can only see their own records)"""
        try:
            query = select(Share_links).where(Share_links.id == obj_id)
            if user_id:
                query = query.where(Share_links.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching share_links {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of share_linkss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Share_links.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Share_links, field):
                        column = getattr(Share_links, field)
            ...
            stmt = select(Share_links, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Share_links, field_name):
                        stmt = stmt.order_by(getattr(Share_links, field_name).desc())
                else:
                    if hasattr(Share_links, sort):
                        stmt = stmt.order_by(getattr(Share_links, sort))
            else:
                stmt = stmt.order_by(Share_links.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Share_links)
                for condition in conditions:
            ...
            logger.error(f"Error fetching share_links list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None, *, commit: bool = True) -> Optional[Share_links]:
        """Update share_links (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Share_links {obj_id} not found for update")
                return None
            ...
            if commit:
                ...
                await self.db.flush()
            await self.db.refresh(obj)
            logger.info(f"Updated share_links {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating share_links {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None, *, commit: bool = True) -> bool:
        """Delete share_links (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Share_links {obj_id} not found for deletion")
                return False
            ...
            if commit:
                ...
                await self.db.flush()
            logger.info(f"Deleted share_links {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting share_links {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Share_links]:
        """Get share_links by any field"""
        try:
            if not hasattr(Share_links, field_name):
                raise ValueError(f"Field {field_name} does not exist on Share_links")
            column = getattr(Share_links, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Share_links).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching share_links by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Share_links]:
        """Get list of share_linkss filtered by field"""
        try:
            if not hasattr(Share_links, field_name):
                raise ValueError(f"Field {field_name} does not exist on Share_links")
            column = getattr(Share_links, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Share_links.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching share_linkss by {field_name}: {str(e)}")
            raise
