from services.project_integrity import require_project, validate_record
...
# ------------------ Service Layer ------------------
class App_recordsService:
    """Service layer for App_records operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None, *, commit: bool = True) -> Optional[App_records]:
        """Create a new app_records"""
        try:
            project = await require_project(self.db, data.get("project_id"), user_id)
            await validate_record(self.db, project, data, user_id)
            if user_id:
                data['user_id'] = user_id
            obj = App_records(**data)
            self.db.add(obj)
            if commit:
            ...
            logger.info(f"Created app_records with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating app_records: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for app_records {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[App_records]:
        """Get app_records by ID (user can only see their own records)"""
        try:
            query = select(App_records).where(App_records.id == obj_id)
            if user_id:
                query = query.where(App_records.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching app_records {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of app_recordss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(App_records.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(App_records, field):
                        column = getattr(App_records, field)
            ...
            stmt = select(App_records, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(App_records, field_name):
                        stmt = stmt.order_by(getattr(App_records, field_name).desc())
                else:
                    if hasattr(App_records, sort):
                        stmt = stmt.order_by(getattr(App_records, sort))
            else:
                stmt = stmt.order_by(App_records.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(App_records)
                for condition in conditions:
            ...
            logger.error(f"Error fetching app_records list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None, *, commit: bool = True) -> Optional[App_records]:
        """Update app_records (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"App_records {obj_id} not found for update")
                return None
            ...
            project = await require_project(self.db, obj.project_id, user_id)
            for immutable in ('collection_key', 'record_key'):
                if immutable in update_data and update_data[immutable] != getattr(obj, immutable):
                    raise HTTPException(status_code=400, detail="不能更改记录所属集合或标识。")
            if 'data' in update_data:
                await validate_record(self.db, project, {'collection_key': obj.collection_key, 'data': update_data['data']}, user_id)
            ...
            if commit:
                ...
                await self.db.flush()
            await self.db.refresh(obj)
            logger.info(f"Updated app_records {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating app_records {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None, *, commit: bool = True) -> bool:
        """Delete app_records (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"App_records {obj_id} not found for deletion")
                return False
            ...
            if commit:
                ...
                await self.db.flush()
            logger.info(f"Deleted app_records {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting app_records {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[App_records]:
        """Get app_records by any field"""
        try:
            if not hasattr(App_records, field_name):
                raise ValueError(f"Field {field_name} does not exist on App_records")
            column = getattr(App_records, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(App_records).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching app_records by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[App_records]:
        """Get list of app_recordss filtered by field"""
        try:
            if not hasattr(App_records, field_name):
                raise ValueError(f"Field {field_name} does not exist on App_records")
            column = getattr(App_records, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(App_records.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching app_recordss by {field_name}: {str(e)}")
            raise
