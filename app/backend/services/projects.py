from services.project_integrity import require_version
...
class ProjectsService:
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None, *, commit: bool = True) -> Optional[Projects]:
        """Create a new projects"""
        try:
            if user_id:
                data['user_id'] = user_id
            if data.get('active_version_id') is not None:
                raise HTTPException(status_code=400, detail="新项目不能引用已有版本。")
            obj = Projects(**data)
            self.db.add(obj)
            if commit:
    ...
    async def get_list(
        ...
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Projects.user_id == user_id)
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None, *, commit: bool = True) -> Optional[Projects]:
